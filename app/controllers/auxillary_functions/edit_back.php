...
**Purpose: This php script is meant to querying the database and edit/update an existing value based on the table fields passed. The edit/update function in this script is generic and will work on any table provided the proper data.
*/
...
function edit_sql($tablename, $primary_key, $table_field_array)
{
	...
	//This loop will iterate through every key and generate a comma separated key(s) and the new value(s) that is to be updated. This will allow for a generic insert query.
	for($i=0; $i < count($keys_array); $i++) 
	{
		//mysqli_real_escape_string will account for the sql escape characters to be handled properly. The comma_separated_key_val will hold a new string with key[0]=value[0], ... key[n-1]=key[n-1]
	...
	{
		//Since the primary key was passed, we will query the table based on the primary key to return the updated data from the row. 
