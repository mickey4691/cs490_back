...
**Purpose: This php script is meant to query the database and list data 
based on the table fields and order passed. 
The list function in this script is generic 
and will work on any table provided the proper data.
*/
...
function list_sql($tablename, $order, $order_by, $table_field_array)
{
	...
	//filter_by and it's corresponding comma separated value. 
	//This will allow for a generic list query.
	for($i=0; $i < count($keys_array); $i++) 
	{
		//mysqli_real_escape_string will account for the sql escape characters 
		//to be handled properly. The comma_separated_filer will hold a string
		//with key[0]=value[0], ... , key[n-2]=key[n-2] AND key[n-1]=key[n-1] 
	...
	//Since the query will have an optional field, that is filter by 
	//ascending/descending order and the order type, 
	//a query will be required to account for that.
	...
	{	
		...
		$i=0;	//initialize variable
    		...
    		{
    			//placing each row from the selected queries to be 
    			//added to an array, iterating through each
        		$db_response[$i]=$row;
        		$i++; //Increment counter for the array
    		}
    		//Generating a successful response with all the rows as an 
    		//array returned from the database.    		
			$response = array
			(
    			"action" => "list",
    			"status" => "success" ,
	...
	else
	{
		...
		(
    		...
    		"internal_message" => "list_back.php Failed to list from 
